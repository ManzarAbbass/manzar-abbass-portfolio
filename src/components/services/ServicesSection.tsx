...
function ServiceCard({
  ...
  return (
    <motion.div
      className="group relative flex flex-col border border-white/10 p-6 transition-colors duration-300 hover:border-accent/30 sm:p-8 md:p-10"
      whileHover={{ y: -4, boxShadow: "0 0 30px rgba(234,179,8,0.08)" }}
      transition={{ type: "spring", stiffness: 300, damping: 20 }}
    >
      <div className="absolute inset-0 bg-gradient-to-b from-accent/0 to-accent/0 opacity-0 transition-opacity duration-300 group-hover:from-accent/[0.02] group-hover:to-accent/0 group-hover:opacity-100" />
      <div className="relative flex items-center justify-between">
        <span className="font-mono text-sm text-accent/60">{id}</span>
        <div className="h-8 w-8 rounded-sm border border-accent/20 transition-colors duration-300 group-hover:border-accent/40">
      ...
      <ul className="relative mt-6 space-y-2.5">
        {bullets.map((bullet, i) => (
          <li
            key={bullet}
            className="flex items-start gap-2.5 font-mono text-sm text-muted transition-all duration-300 group-hover:text-gray-300"
            style={{ transitionDelay: `${i * 50}ms` }}
          >
            <span className="mt-0.5 shrink-0 text-accent transition-transform duration-300 group-hover:scale-110">
    ...
    <section id="services" className="relative border-b border-white/20">
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-24 md:py-32">
        <div className="flex items-center gap-4">
          <span className="font-mono text-sm tracking-[0.2em] text-accent">02</span>
